...
export async function POST(req: Request) {
  ...
  if (!setting && logoFileName && logoBuffer) {
    ...
    const [{ request: logoFile }, { request: bannerFile }] = await Promise.all([
    ...
    if (setting && logo && logoFileName && logoBuffer) {
      ...
      const { request: logoFileBlob } = await upload(
          "settings",
          logoFileName,
          logoBuffer
        ),
    ...
    if (setting && banner && bannerBuffer && bannerFileName) {
      ...
      const { request: bannerFileBlob } = await upload(
          "settings",
          bannerFileName,
          bannerBuffer
        ),
