...
export default function QuizNavigation({
  ...
  return (
    <Card {...props} radius="sm" shadow="none">
      ...
      <CardBody>
        <div className="flex flex-row mx-auto flex-wrap md:grid md:grid-cols-3 lg:grid-cols-5 gap-5 w-fit">
          {question.map((item: any, index: number) => {
            const answered = formData.find(
              (data: any) => data.title === item.title
            );
            return (
              <NextLink
                href={`#${index + 1}`}
                ...
                key={index}
                className={`bg-white items-center flex justify-between flex-col w-8 h-8 md:w-10 md:h-10`}
              >
      ...
      <CardFooter>
        <Button onClick={handleSubmit} className="w-full bg-white" radius="sm">
          Finish Attempt
        </Button>
      </CardFooter>
    </Card>
